/**
 * 微信小游戏桥接适配器（aigame-framework）
 *
 * 提供 three.js 需要的最小 window/document/canvas 环境，并补齐：
 * - fetch：读本地包内 JSON（config/manifest/关卡表），读不到再走网络 wx.request
 * - Image：wx.createImage 桥接，让 three 的 TextureLoader 能加载贴图
 * - 触摸 → 指针事件：游戏代码统一用 pointerdown/up/move，不用写 wx 专用逻辑
...
const sysInfo =
  wxGlobal && wxGlobal.getSystemInfoSync
    ? wxGlobal.getSystemInfoSync()
    : { windowWidth: 720, windowHeight: 1280, pixelRatio: 1 };
const canvas = wxGlobal && wxGlobal.createCanvas ? wxGlobal.createCanvas() : null;

if (!canvas) {
  console.warn("[aigame-adapter][wx] createCanvas 不可用，Three.js 可能无法初始化");
}
...
const createImage = wxGlobal && wxGlobal.createImage ? () => wxGlobal.createImage() : null;
...
const storageBridge = {
  getItem(key) {
    if (!wxGlobal || !wxGlobal.getStorageSync) return null;
    const value = wxGlobal.getStorageSync(key);
    return value === "" ? null : String(value);
  },
  setItem(key, value) {
    if (!wxGlobal || !wxGlobal.setStorageSync) return;
    wxGlobal.setStorageSync(key, String(value));
  }
};

// fetch：优先读本地包内文件（同步，JSON 配置等），读不到再走网络 wx.request
const fileSystem = wxGlobal && wxGlobal.getFileSystemManager ? wxGlobal.getFileSystemManager() : null;
rootGlobal.fetch = function fetch(url) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (wxGlobal && wxGlobal.request) {
      ...
      reject(new Error("[aigame-adapter][wx] fetch 不可用: " + url));
    }
  });
};
...
rootGlobal.navigator = rootGlobal.navigator || { userAgent: "wechat-minigame" };
...
console.info("[aigame-adapter][wx] runtime ready", {
